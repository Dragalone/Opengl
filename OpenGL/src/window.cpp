...
void Window::do_movement()
{
	...
	if (keys[GLFW_KEY_W])
		currentScene.cameraPosition += cameraSpeed * currentScene.cameraFront;
	if (keys[GLFW_KEY_S])
		currentScene.cameraPosition -= cameraSpeed * currentScene.cameraFront;
	if (keys[GLFW_KEY_A])
		currentScene.cameraPosition -= glm::normalize(glm::cross(currentScene.cameraFront, currentScene.cameraUp)) * cameraSpeed;
	if (keys[GLFW_KEY_D])
		currentScene.cameraPosition += glm::normalize(glm::cross(currentScene.cameraFront, currentScene.cameraUp)) * cameraSpeed;
}

void Window::do_movement_object() {
	if (keys[GLFW_KEY_UP]) {
		for (auto &i : currentScene.sceneObjects) {
			if (i.simpleAnimation) {
				i.angle += glm::radians(3.f);
				i.position += glm::vec3(0.0f, 0.0f, 0.1f);
			}
		}
	...
	if (keys[GLFW_KEY_LEFT]){
		for (auto &i : currentScene.sceneObjects) {
			if (i.simpleAnimation) {
				i.angle += glm::radians(3.f);
				i.position += glm::vec3(-0.1f, 0.0f, 0.0f);
			}
		}
	}
...
void Window::render()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		do_movement();
		do_movement_object();
		if (GLFW_PRESS == glfwGetKey(window,
			GLFW_KEY_ESCAPE))
		{
